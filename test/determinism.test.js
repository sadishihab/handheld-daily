...
import { dailySeed, puzzleNumber, msUntilNextPuzzle, utcDateString } from '../src/daily.js';
...
function createDummySim(seed) {
  ...
  return {
    trace,
    update(stepIndex) {
      state.x += rng.nextInt(-3, 4);
      state.drift = state.drift * 0.99 + rng.nextFloat();
      if (stepIndex % 100 === 0) state.bag.push(rng.nextInt(0, 1000));
...
while (jitterLoop.stepCount < STEPS) {
  jitterLoop.advance(jitterRng.nextInt(0, 40));
}
...
for (let i = 0; i < 60000; i++) {
  const v = ints.nextInt(0, 6);
  if (!Number.isInteger(v) || v < 0 || v >= 6) intsInRange = false;
  histogram[v] += 1;
}
check('nextInt stays in [min, max)', intsInRange, `histogram ${histogram.join()}`);
// 10000 expected per bucket; a badly biased generator misses by far more.
check(
  'nextInt is roughly uniform',
...
check('launch day is puzzle #1', puzzleNumber(Date.parse('2026-01-01T00:00:00Z')) === 1);
check('the day after launch is puzzle #2', puzzleNumber(Date.parse('2026-01-02T00:00:00Z')) === 2);
check(
